...
class StreamingServer:
    ...
    def __key_exchange(self):
        sender_username = 59
        reveiver_username = 61
        P = 33
        G = 8
        a = 3
        A = pow(G, a, P)
        A_str = str(A)
        A_shahash = hashlib.sha256(A_str.encode()).hexdigest()
        sender_publicKey, sender_privateKey = rsa.newkeys(512)
        A_enc = rsa.encrypt(message.encode(),sender_publicKey)
        packet = (A,A_enc,sender_publicKey)
...
class ZoomClone:
    def __init__(self,local_ip_address):
        ...
        self.server = StreamingServer(local_ip_address, 9999)
        self.receiver = AudioReceiver(local_ip_address, 8888)
    ...
    def start_camera_stream(self,client_ip_address):
        self.camera_client = CameraClient(client_ip_address, 7777)
    ...
    def start_screen_sharing(self,client_ip_address):
        self.screen_client = ScreenShareClient(client_ip_address, 7777)
    ...
    def start_audio_stream(self,client_ip_address):
        self.audio_sender = AudioSender(client_ip_address, 6666)
...
window = tk.Tk()
window.title("Video Call from Device 1")
